...
namespace MBox
{
    namespace Vol
    {
        namespace Security
        {
            ...
            static HRESULT FormatStringSidToSid(
                const wchar_t* aStringSid,
                Sid* aSid,
                UINT32* aNeedBytes)
            {
            ...
            static HRESULT FormatStringAclToAcl(
                bool aIsSacl, // true == Sacl, false == Dacl
                const wchar_t* aStringAcl,
                UINT32* aFlags,
                ...
                UINT32* aNeedBytes)
            {
                ...
                *aFlags = ParseStringAclFlags(aIsSacl, &aStringAcl);
                ...
                while (*aStringAcl == L'(')
                {
                    ...
                    hr = FormatStringSidToSid(
                        aStringAcl, vAce ? (Sid*)(&vAce->m_SidStart) : nullptr, &vSidNeedBytes);
                    if (SUCCEEDED(hr) || E_NOT_SUFFICIENT_BUFFER == hr)
                    {
            ...
            static HRESULT FormatSddlToSecurityDescriptor(
                const wchar_t* aSddl,
                ...
                UINT32* aSecurityDescriptorNeedBytes)
            {
                ...
                *aSecurityDescriptorNeedBytes = sizeof(SecurityDescriptor);
                ...
                vSddlToken = new(std::nothrow) wchar_t[wcslen(aSddl) + 1]{};
                ...
                while (*aSddl)
                {
                    ...
                    case L'O':
                    {
                        hr = FormatStringSidToSid(vSddlToken, (Sid*)vNextOffset, &vNeedBytes);
                        if (FAILED(hr)
                            && E_NOT_SUFFICIENT_BUFFER != hr)
                        {
                            break;
                    ...
                    case L'D':
                    {
                        ...
                        hr = FormatStringAclToAcl(false, vSddlToken, &vAclFlags, (AccessControlList*)vNextOffset, &vNeedBytes);
                        if (FAILED(hr)
                            && E_NOT_SUFFICIENT_BUFFER != hr)
                        {
                            break;
                    ...
                    case L'S':
                    {
                        ...
                        hr = FormatStringAclToAcl(true, vSddlToken, &vAclFlags, (AccessControlList*)vNextOffset, &vNeedBytes);
                        if (FAILED(hr)
                            && E_NOT_SUFFICIENT_BUFFER != hr)
                        {
                            break;
            ...
            HRESULT BuildSid(const wchar_t * aStringSid, Sid ** aSid)
            {
                ...
                {
                    UINT32 vNeedBytes = 0;
                    hr = FormatStringSidToSid(aStringSid, nullptr, &vNeedBytes);
                    if (E_NOT_SUFFICIENT_BUFFER != hr)
                    {
                        break;
                    }

                    vSid = (Sid*)new(std::nothrow) unsigned char[vNeedBytes] {};
                    if (nullptr == vSid)
                    {
                        hr = E_NOT_SUFFICIENT_BUFFER;
                        break;
                    ...
                    hr = FormatStringSidToSid(aStringSid, vSid, &vNeedBytes);
                    if (FAILED(hr))
                    {
                        break;
                    }
                ...
                UINT32 * aSecurityDescriptorBytes)
            {
                ...
                for (;;)
                {
                    ...
                    {
                        UINT32 vNeedBytes = 0;
                        hr = FormatSddlToSecurityDescriptor(aSddl, nullptr, &vNeedBytes);
                        if (E_NOT_SUFFICIENT_BUFFER != hr)
                        {
                            break;
                        }
                        ...
                        hr = FormatSddlToSecurityDescriptor(aSddl, vSecurityDescriptor, &vNeedBytes);
                        if (FAILED(hr))
                        {
                            break;
                        }
